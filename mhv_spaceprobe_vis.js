...
$(document).ready(function() { // Get latest 6 tweets by jQueryHowto
	$.jTwitter('makehackvoid', 400, function(data) {
		$('#posts').empty();
		$.each(data, function(i, post) { // possible matches (~hh:mm) is the estimated closing time
...
function drawGraph(data) { // function to draw the graph
	...
	var x = d3.scale.linear() // .domain([0, data.length])
		.domain([1, 7])
		.range([20, w - 20]); // var y = d3.scale.linear()
	// 	.domain([0, 24.0])
	// 	.rangeRound([h, 0]);
	...
	circle.enter()
		.append("svg:circle") //	 .attr("cy", function(d) { return y((d.getHours() + (d.getMinutes()/60.0))); })
